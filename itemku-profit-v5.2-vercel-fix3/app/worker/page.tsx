...
export default function WorkerPage(){
  ...
  return <main className="min-h-screen bg-background pb-24 text-foreground md:pb-6">
    ...
    <nav className="fixed inset-x-0 bottom-0 z-30 grid grid-cols-3 border-t bg-card/95 p-2 backdrop-blur md:hidden">{(['Aktif','Selesai','Refund'] as const).map(x=><button key={x} onClick={()=>setFilter(x)} className={`rounded-xl px-2 py-3 text-xs font-black ${filter===x?'bg-primary text-primary-foreground':'text-muted-foreground'}`}>{x}</button>)}</nav>
  </main>
}
