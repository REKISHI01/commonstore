...
export async function GET(req:Request){
  try{
    ...
    const [products,orders,opps,audit,restocks,expenses,targets,history,customerNotes,profiles,channels,suppliers,pos,ledger,settlements,disputes,rules,notifications,prefs,tags]=await Promise.all([
      q('products','*','created_at.asc'),q('orders','*','created_at.asc'),q('opportunities','*','checked_at.desc'),q('audit_logs','*','created_at.desc'),q('restocks','*','created_at.desc'),q('expenses','*','expense_date.desc'),q('business_targets','*','month_key.desc'),q('price_history','*','created_at.desc'),q('customer_notes','*','updated_at.desc'),q('profiles','id,name,role,active,permissions,allowed_games','created_at.asc'),
      user.role==='owner'?q('channel_rules','*','name.asc'):Promise.resolve([]),user.role==='owner'?q('suppliers','*','name.asc'):Promise.resolve([]),user.role==='owner'?q('purchase_orders','*','created_at.desc'):Promise.resolve([]),user.role==='owner'?q('inventory_ledger','*','created_at.desc'):Promise.resolve([]),user.role==='owner'?q('settlements','*','created_at.desc'):Promise.resolve([]),user.role==='owner'?q('disputes','*','created_at.desc'):Promise.resolve([]),user.role==='owner'?q('automation_rules','*','kind.asc'):Promise.resolve([]),user.role==='owner'?q('notifications','*','created_at.desc'):Promise.resolve([]),user.role==='owner'?q('dashboard_preferences','*','updated_at.desc'):Promise.resolve([]),user.role==='owner'?q('customer_tags','*','updated_at.desc'):Promise.resolve([]),
    ])
